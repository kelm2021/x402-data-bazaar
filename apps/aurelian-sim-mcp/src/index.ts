...
function coerceNumericRecord(value: unknown): Record<string, number> | undefined {
...
function coerceScenarioLike(value: unknown): unknown {
  if (!isPlainObject(value)) {
    return value;
  }
...
function unwrapToolPayload(input: unknown): unknown {
  let current = input;
...
function normalizeScenario(input: unknown, fieldName: string) {
...
function extractScenarioFromPayload(payload: JsonObject, fieldName: string) {
...
function readStringAlias(payload: JsonObject, keys: string[]) {
...
async function callSim(path: string, args: unknown) {
  try {
...
function parseProbabilityArgs(rawArgs: unknown) {
...
function parseCompareArgs(rawArgs: unknown) {
  const payload = unwrapToolPayload(rawArgs);
  if (!isPlainObject(payload)) {
...
function parseSensitivityArgs(rawArgs: unknown) {
  const payload = unwrapToolPayload(rawArgs);
  if (!isPlainObject(payload)) {
...
function parseForecastArgs(rawArgs: unknown) {
  const payload = unwrapToolPayload(rawArgs);
  if (!isPlainObject(payload)) {
  ...
  const parsed = {
    ...
    growth_mode?: "additive" | "multiplicative";
  };
  ...
  const uncertaintyGrowth = coerceFiniteNumber(
    payload.uncertainty_growth ?? payload.uncertaintyGrowth,
  );
...
function parseComposedArgs(rawArgs: unknown) {
  const payload = unwrapToolPayload(rawArgs);
  if (!isPlainObject(payload)) {
...
function parseOptimizeArgs(rawArgs: unknown) {
  const payload = unwrapToolPayload(rawArgs);
  if (!isPlainObject(payload)) {
  ...
  const parsed = {
    ...
    iterations?: number;
    objective?: "outcome_probability" | "mean_score";
  };
  ...
  const iterations = coerceFiniteNumber(
    payload.iterations ?? payload.iteration_count ?? payload.max_iterations,
  );
...
server.tool(
  "run_probability_sim",
  "Estimate outcome probability for a scenario using Monte Carlo simulation over parameter uncertainty.",
  toolEnvelopeShape,
  async (args) => callSim("/api/sim/probability", parseProbabilityArgs(args)),
);

server.tool(
  "compare_scenarios",
  "Compare baseline and candidate scenarios and return uplift deltas with diagnostics.",
  toolEnvelopeShape,
  async (args) => callSim("/api/sim/compare", parseCompareArgs(args)),
);

server.tool(
  "run_sensitivity",
  "Measure sensitivity of one parameter via plus/minus perturbation runs.",
  toolEnvelopeShape,
  async (args) => callSim("/api/sim/sensitivity", parseSensitivityArgs(args)),
);

server.tool(
  "forecast_probability",
  "Generate a forward probability path across future periods under trend assumptions.",
  toolEnvelopeShape,
  async (args) => callSim("/api/sim/forecast", parseForecastArgs(args)),
);

server.tool(
  "compose_scenarios",
  "Blend weighted scenario components into a single probability estimate.",
  toolEnvelopeShape,
  async (args) => callSim("/api/sim/composed", parseComposedArgs(args)),
);

server.tool(
  "optimize_scenario",
  "Search bounded parameter ranges to maximize objective value.",
  toolEnvelopeShape,
  async (args) => callSim("/api/sim/optimize", parseOptimizeArgs(args)),
);
}
