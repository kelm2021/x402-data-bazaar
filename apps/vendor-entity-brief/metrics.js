    ...
    async getSummary() {
      ...
      if (sourceKeys.length > 0) {
        ...
        for (const sourceId of sourceKeys) {
          sourcePipeline.hgetall(getMetricsRedisKeys(null, null, sourceId).source);
        }
      ...
      if (serviceHosts.length > 0) {
        ...
        for (const serviceHost of serviceHosts) {
          const serviceKeys = getMetricsRedisKeys(null, null, null, serviceHost);
...
function renderDashboardPage(summary, options = {}) {
  ...
  const trafficQuality = summary.trafficQuality ?? {};
  const nonPayingRequestShare = trafficQuality.requests?.shareNonPaying ?? 0;
  const nonPaying402Share = trafficQuality.paymentRequired?.shareFromNonPayingSources ?? 0;
  const nonPayingSourceCount = trafficQuality.sources?.nonPaying ?? 0;
  const activePaying24h = trafficQuality.sources?.activePaying24h ?? 0;
  const activePaying72h = trafficQuality.sources?.activePaying72h ?? 0;
  ...
  const cards = [
    ...
    renderSummaryCard(
      ...
      `${formatInteger(revenueActiveServices)} observed hosts have external revenue`,
    ),
    renderSummaryCard(
      "Non-paying traffic share",
      formatPercent(nonPayingRequestShare),
      `${formatInteger(nonPayingSourceCount)} zero-paid sources generated ${formatPercent(nonPaying402Share)} of 402 responses`,
      { className: "metric-card-warm" },
    ),
    renderSummaryCard(
      "Active paying sources",
      `${formatInteger(activePaying24h)} / ${formatInteger(activePaying72h)}`,
      "Unique paying source fingerprints in the last 24h / 72h",
    ),
    renderSummaryCard(
  ...
  <body>
    ...
    <div class="page-shell">
      <header>
        ...
        <p class="lede">See which observed hosts and route families are turning marketplace demand into settled revenue, while keeping self-verification traffic separate from external buyers.</p>
        <div class="hero-chips" aria-label="Current leaders">
          <p class="hero-chip"><strong>Top host:</strong> ${escapeHtml(
            formatObservedHostLabel(topService?.serviceHost || "No observed host traffic yet"),
          )}</p>
      ...
      <main id="metrics-main">
        ...
        <section aria-labelledby="notes-heading">
          <div class="section-header">
            <div>
              ...
              <p>The dashboard leads with buyer revenue first, then shows which observed hosts and route families are converting that demand.</p>
            </div>
          </div>
        ...
        <section aria-labelledby="services-heading">
          <div class="section-header">
            <div>
              <h2 id="services-heading">Observed Hosts</h2>
              <p>Shared metrics grouped by request host so you can debug attribution. Observed-host counts are not the same thing as product counts until host normalization is cleaned up.</p>
            </div>
          </div>
