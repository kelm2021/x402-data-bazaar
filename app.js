...
function createDiscoveryExtension(options = {}) {
  ...
  // Fix POST route discovery: the @x402/extensions/bazaar schema only allows
  // GET/HEAD/DELETE methods. Patch it to support POST with body params so
  // 402scan can render input fields for POST endpoints.
  if (queryExample && extension?.bazaar?.schema?.properties?.input?.properties?.method) {
    const methodSchema = extension.bazaar.schema.properties.input.properties.method;
    if (Array.isArray(methodSchema.enum) && !methodSchema.enum.includes("POST")) {
      methodSchema.enum = ["GET", "HEAD", "DELETE", "POST", "PUT", "PATCH"];
    }
    // Add bodyParams to schema if not present
    const inputProps = extension.bazaar.schema.properties.input.properties;
    if (!inputProps.bodyParams) {
      inputProps.bodyParams = { type: "object", additionalProperties: true };
    }
    // Allow additional properties on input for bodyParams
    if (extension.bazaar.schema.properties.input.additionalProperties === false) {
      extension.bazaar.schema.properties.input.additionalProperties = true;
    }
  }
...
function createRouteConfig(payTo = PAY_TO) {
  return {
    ...
    // ─── Simulation Routes ──────────────────────────────────
    "POST /api/sim/probability": createPricedRoute({ price: "0.05", description: "Monte Carlo probability estimation", category: "simulation", tags: ["simulation", "monte-carlo"], payTo, queryExample: { parameters: { demand: 0.72, execution: 0.65, pressure: -0.35 }, weights: { demand: 1.2, execution: 1.0, pressure: 0.9 }, uncertainty: { demand: 0.12, execution: 0.1, pressure: 0.2 }, threshold: 0.25 } }),
    "POST /api/sim/compare": createPricedRoute({ price: "0.06", description: "Scenario comparison with uplift deltas", category: "simulation", tags: ["simulation", "scenario-compare"], payTo, queryExample: { baseline: { parameters: { a: 0.6, b: 0.5 }, threshold: 0.25 }, candidate: { parameters: { a: 0.8, b: 0.7 }, threshold: 0.25 } } }),
    "POST /api/sim/sensitivity": createPricedRoute({ price: "0.07", description: "Parameter sensitivity analysis", category: "simulation", tags: ["simulation", "sensitivity"], payTo, queryExample: { scenario: { parameters: { a: 0.7, b: 0.6 }, threshold: 0.25 }, parameter: "a", delta: 0.1, mode: "relative" } }),
    "POST /api/sim/forecast": createPricedRoute({ price: "0.08", description: "Forward probability path forecast", category: "simulation", tags: ["simulation", "forecast"], payTo, queryExample: { scenario: { parameters: { a: 0.7, b: 0.6 }, threshold: 0.25 }, periods: 4, drift: { a: 0.02, b: 0.01 }, uncertainty_growth: 0.02 } }),
    "POST /api/sim/composed": createPricedRoute({ price: "0.09", description: "Weighted scenario composition", category: "simulation", tags: ["simulation", "ensemble"], payTo, queryExample: { components: [{ label: "bull", weight: 0.6, scenario: { parameters: { signal: 0.8 }, threshold: 0.3 } }, { label: "bear", weight: 0.4, scenario: { parameters: { signal: -0.3 }, threshold: 0.3 } }] } }),
    "POST /api/sim/optimize": createPricedRoute({ price: "0.10", description: "Parameter optimization search", category: "simulation", tags: ["simulation", "optimization"], payTo, queryExample: { scenario: { parameters: { a: 0.7, b: 0.5 }, threshold: 0.3 }, bounds: { a: { min: 0.4, max: 1.0 }, b: { min: 0.2, max: 0.9 } }, iterations: 100 } }),

    // ─── Document Generation Routes ─────────────────────────
    "POST /api/tools/docx/generate": createPricedRoute({ price: "0.015", description: "Generate DOCX documents", category: "document-generation", tags: ["docx"], payTo, queryExample: { template: "general", title: "Quarterly Report", company: { name: "Acme Corp" }, sections: [{ title: "Summary", content: "Overview of Q1 results." }] } }),
    "POST /api/tools/xlsx/generate": createPricedRoute({ price: "0.015", description: "Generate XLSX spreadsheets", category: "document-generation", tags: ["xlsx"], payTo, queryExample: { template: "invoice", company: { name: "Acme Corp" }, items: [{ description: "Consulting", quantity: 10, price: 150 }], title: "Invoice #1001" } }),
    "POST /api/tools/invoice/generate": createPricedRoute({ price: "0.015", description: "Generate PDF invoices", category: "document-generation", tags: ["pdf", "invoice"], payTo, queryExample: { from: { name: "Acme Corp", address: "123 Main St" }, to: { name: "Client Inc", address: "456 Oak Ave" }, invoice_number: "INV-1001", date: "2026-04-01", items: [{ description: "Service", quantity: 1, price: 500 }], tax_rate: 8.5 } }),
